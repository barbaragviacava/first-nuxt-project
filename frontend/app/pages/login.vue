  ...
  <ValidationObserver ref="form">
    <form method="POST" @submit.prevent="onSubmit">
      ...
      <div class="login-buttons">
        <BaseButton type="submit" class="btn h-45px btn-success d-block w-100 btn-lg" :loading="isLoading">{{ $t('pages.login.accessButton') }}</BaseButton>
      </div>
    </form>
  </ValidationObserver>
</template>
...
export default {
  ...
  data() {
    return {
      labels: {
        email : 'E-mail',
        password : this.$t('pages.login.password'),
      },
      user: {
        email: '',
        password: '',
